...
/**
 * PS_Live_Debug_Helper Class.
 */
if ( ! class_exists( 'PS_Live_Debug_Helper' ) ) {
	class PS_Live_Debug_Helper {

		/**
		 * PS_Live_Debug_Helper constructor.
		 *
		 * @uses PS_Live_Debug_Helper::init()
		 *
		 * @return void
		 */
		...
		public static function get_first_backup() {
			if ( file_exists( PS_LIVE_DEBUG_WP_CONFIG ) ) {
				copy( PS_LIVE_DEBUG_WP_CONFIG, PS_LIVE_DEBUG_WP_CONFIG_BACKUP_ORIGINAL );
			}
		}
		 ...
		 */
		public static function table_psource_constants( $array ) {
			$table = '<table class="sui-table striped"><thead><tr><th>' . esc_html__( 'Title', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Default Value', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Value', 'ps-live-debug' ) . '</th></tr></thead><tbody>';
			...
			$table .= '<tfoot><tr><th>' . esc_html__( 'Title', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Default Value', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Value', 'ps-live-debug' ) . '</th></tr></tfoot>';
			$table .= '</tbody></table>';
		 ...
		 * @return string html of the table.
		 */
		public static function table_psource_actions_filters( $array ) {
			$table  = '<table class="sui-table striped"><thead><tr><th>' . esc_html__( 'Actions', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Filters', 'ps-live-debug' ) . '</th></tr></thead><tbody>';
			$table .= '<tr><td>';
			...
			$table .= '</td><tr>';
			$table .= '<tfoot><tr><th>' . esc_html__( 'Actions', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Filters', 'ps-live-debug' ) . '</th></tr></tfoot>';
			$table .= '</tbody></table>';
		...
		public static function table_general( $array ) {
			$table = '<table class="sui-table striped"><thead><tr><th>' . esc_html__( 'Title', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Value', 'ps-live-debug' ) . '</th></tr></thead><tbody>';

			foreach ( $array as $key => $value ) {
				$table .= '<tr><td>' . esc_html( $key ) . '</td><td>' . $value . '</td></tr>';
			}

			$table .= '<tfoot><tr><th>' . esc_html__( 'Title', 'ps-live-debug' ) . '</th><th>' . esc_html__( 'Value', 'ps-live-debug' ) . '</th></tr></tfoot>';
			$table .= '</tbody></table>';
		...
		public static function format_constant( $constant ) {
			if ( ! defined( $constant ) ) {
				return '<em>' . esc_html__( 'Undefined', 'ps-live-debug' ) . '</em>';
			}
